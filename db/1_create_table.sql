...
CREATE TABLE "3DSLICESERVER"."PERMISSION" (
	...
	"CREATE_PART" boolean DEFAULT TRUE,
	"CREATE_USER" boolean DEFAULT FALSE,
	"MODIFY_USER" boolean DEFAULT FALSE,
	"SUPRESS_USER" boolean DEFAULT FALSE,
	"CREATE_GROUP" boolean DEFAULT FALSE,
	"MODIFY_GROUP" boolean DEFAULT FALSE,
	"SUPRESS_GROUP" boolean DEFAULT FALSE,
	"CREATED_AT" timestamp without time zone NOT NULL DEFAULT now(),
	"UPDATED_AT" timestamp without time zone NOT NULL DEFAULT now(),
...
CREATE TABLE "3DSLICESERVER"."PERMISSION_PART" (
	"ID" SERIAL PRIMARY KEY NOT NULL,
	"USER_ID" integer DEFAULT NULL, -- foreign key
	"GROUP_ID" integer DEFAULT NULL, -- foreign key
...
ALTER SEQUENCE "3DSLICESERVER"."PERMISSION_PART_ID_seq" RESTART WITH 1453;

CREATE TRIGGER set_timestamp_permission_part
BEFORE UPDATE ON "3DSLICESERVER"."PERMISSION_PART"
FOR EACH ROW
	EXECUTE PROCEDURE trigger_set_timestamp();
...
CREATE TABLE "3DSLICESERVER"."SLICE_SPECIFICATION" (
	"ID" SERIAL PRIMARY KEY NOT NULL,
	"CURA_DEFINITION_FILE_E0" varchar(255) DEFAULT NULL,
	"CURA_DEFINITION_FILE_E1" varchar(255) DEFAULT NULL,
	"CREATED_AT" timestamp without time zone NOT NULL DEFAULT now(),
	"UPDATED_AT" timestamp without time zone NOT NULL DEFAULT now()
);

ALTER SEQUENCE "3DSLICESERVER"."SLICE_SPECIFICATION_ID_seq" RESTART WITH 1453;

CREATE TRIGGER set_timestamp_slice
BEFORE UPDATE ON "3DSLICESERVER"."SLICE_SPECIFICATION"
FOR EACH ROW
	EXECUTE PROCEDURE trigger_set_timestamp();

CREATE TABLE "3DSLICESERVER"."CURA_PARAMETER" (
	"ID" SERIAL PRIMARY KEY NOT NULL,
	"SLICE_SPECIFICATION_ID" integer NOT NULL,
	"NO_EXTRUDER" integer NOT NULL,
	"KEY" varchar(100) NOT NULL,
	"VALUE" varchar(100) NOT NULL,

	CONSTRAINT "SLICE_SPECIFICATION" FOREIGN KEY ("SLICE_SPECIFICATION_ID")
	REFERENCES "3DSLICESERVER"."SLICE_SPECIFICATION" ("ID") MATCH SIMPLE
	ON UPDATE CASCADE
	ON DELETE CASCADE
);

ALTER SEQUENCE "3DSLICESERVER"."CURA_PARAMETER_ID_seq" RESTART WITH 1453;

-- TODO faire les foreign key

CREATE TABLE "3DSLICESERVER"."SLICE" (
	"ID" SERIAL PRIMARY KEY NOT NULL,
